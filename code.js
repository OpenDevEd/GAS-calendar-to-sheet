function onOpen() {
  SpreadsheetApp.getUi().createMenu('bCal2Sheet')
    .addItem('Collect calendar information', 'launch')
    .addToUi();
}

function onOpen_calendarToSheet() {
...
function onOpenTrigger() {
  ...
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getEventType() == 'ON_OPEN' && triggers[i].getHandlerFunction() == 'onOpen_calendarToSheet') {
      triggerExists = true;
      break;
    }
  }
  if (triggerExists === false) {
    ...
    ScriptApp.newTrigger('onOpen_calendarToSheet')
